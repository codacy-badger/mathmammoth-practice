...
<html lang="en-US">
  <head>
    ...
    <title>Angles and Their Measures Matching Game &mdash; Online Practice (4th, 5th, and 6th grade)</title>
    <style>
      .flex-container {
        width: 800px;
        max-width: 100%;
    ...
    <meta property="og:title" content="Angles and Their Measures Matching Game">
    <meta property="og:description" content="Match angles to their angle measures, and thereby learn better to estimate angles!">
    <meta property="og:image" content="https://www.mathmammoth.com/practice/match-angles.png">
    <meta property="og:image:secure_url" content="https://www.mathmammoth.com/practice/match-angles.png" />
    <meta property="og:image:type" content="image/png" />
    <meta property="og:image:width" content="638" />
    <meta property="og:image:height" content="542" />
    <meta property="og:image:alt" content="Angles and Their Measures Matching Game" />
    <meta property="og:url" content="https://www.mathmammoth.com/practice/angles-matching">
    <meta property="og:site_name" content="Math Mammoth Practice">
    <meta property="og:locale" content="en_US" />
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:image:alt" content="Angles and Their Measures Matching Game">
  </head>

  <body>
    <?php $page = 'angles-matching'; include 'header.php'; ?>
    <div class="container">
        ...
        <div class="titledesc text-center">
          ...
          <div class="row">
            <div class="col-lg-6">
              ...
              <p>
                Practice the concept of angle measurement in geometry with this fun matching game. 
                You simply match each picture of an angle to its measure. It also helps you to get better at estimating angle measures.
              </p>
              <p>
                You can choose to have either 12 or 18 tiles. Then, the second option makes the game either easier or harder. Choose multiple 
                orientations for the angles to make the game more challenging. This means the angles will be flipped and/or rotated. In other words,
                they don't all appear in the same orientation.
              </p>
              <section class="go p-3 mb-5 shadow-lg" style="border-radius: 15px; box-shadow: 0 1rem 3rem rgba(0,0,0,.4) !important; background-color: #8cff90">
                ...
                <div class="form-group">
                  <div class="custom-control custom-checkbox custom-control-inline">
                    <input type="checkbox" name="orientations" id="orientations" class="custom-control-input">
                    <label for="orientations" class="custom-control-label">Use multiple orientations for angles</label>
                  </div>
                </div>
            ...
            <div class="col-lg-6">
              <p>
                Screenshot:<br />
                <img src="match-angles.png" class="rounded-lg screenshot" alt="Screenshot of someone doing a round">
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="form-group">
              <div class="custom-control custom-checkbox custom-control-inline">
                <input type="checkbox" name="gen-orientations" id="gen-orientations" class="custom-control-input">
                <label for="gen-orientations" class="custom-control-label">Use multiple orientations for angles</label>
              </div>
            </div>
            <div class="form-group">
    ...
    <script>
      ...
      if (params.has("orientations")) {
        $("#orientations").prop("checked", +params.get("orientations") ? true : false);
      }
      // </quick-links> 
      ...
      function generatePairs() {
        ...
        for (var i = 0; i <= numPairs + 2; i++) {
          var degrees = rand(1, 36) * 5;
          while (vals.indexOf(degrees) > -1) {
            degrees = rand(1, 36) * 5;
          }
          vals.push(degrees);
        }
      ...
      document.querySelector('.go-btn').addEventListener('click', function() {
        ...
        orientations = $('#orientations').prop('checked');
        numPairs = gridSize / 2;
      ...
      function setupRound() {
        generatePairs();
        vals = vals.slice(0, numPairs);
        questions = vals.reduce(
          (acc, item, i) =>
            acc.concat(
              { val: item, matchesTo: item, type: 'text' },
              { val: item, matchesTo: item, type: 'pic' }
            ),
          []
        );
        ...
        questions = questions.map(function(item) {
          return {
            ...
            matchesTo: questions.findIndex(function(q) {
              return q.val == item.matchesTo && q.type == (item.type == 'text' ? 'pic' : 'text');
            }),
            type: item.type
          }; 
        });
      ...
      function buildFlexItems() {
        ...
        for (var i = 0; i < items.length; i++) {
          if (questions[i].type == 'text') {
            items[i].innerHTML = questions[i].val + '&deg;';
          } else {
            items[i].innerHTML = '<div class="angle" style="transform: rotate(' + (rand(0, 2) == 2 && orientations ? 180 : 0) + 'deg) rotateY(' + (rand(0, 2) == 2 && orientations ? 180 : 0) + 'deg)"><hr class="first" style="transform: rotate(' + questions[i].val + 'deg)" /><hr /></div>';
          }
      ...
      $('.gen-link-btn').click(function() {
        var link = 'https://www.mathmammoth.com/practice/angles-matching#orientations=' + ($('#gen-orientations').prop("checked") ? 1 : 0) + '&tiles=' + $('#gen-tiles').val();
