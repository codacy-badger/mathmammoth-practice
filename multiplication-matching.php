...
<html lang="en-US">
  <head>
    ...
    <meta
      ...
      content="Practice converting fractions to decimals while also uncovering a hidden picture in this fun matching game!"
    />
  ...
  <body>
        ...
        <div class="titledesc text-center">
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <p>
                You can choose from three different difficulty levels and from three different grid sizes (which determines the
                number of tiles to be matched). For fourth grade, I recommend the easy level, for fifth, the medium level, and for
                sixth and seventh grades the hard level.
              </p>
              <p>
                The easy level only includes fractions with denominators 2, 4, 5, and 10, which give us decimals with one decimal
                digit, plus the decimals 0.25 and 0.75. The medium level uses 20, 25, 50, 100 as denominators, and thus we get
                decimals with two decimal digits. The hard level uses 1000, 500, 8, 200, 100, 40, 50, 250, 125, which match with
                decimal numbers with three decimal places.
              </p>
              <section
              ...
              >
                ...
                <div class="btn-group btn-group-sm">
                  <button
                    ...
                    onclick="$('.tc input').prop('checked', true); $('#submit').prop('disabled', false);"
                  >
                    Check All
                  </button>
                  <button
                    class="btn btn-primary"
                    onclick="$('.tc.in12 input').prop('checked', true); $('#submit').prop('disabled', false);"
                  >
                    Check Tables 1-12
                  </button>
                  <button
                    class="btn btn-primary"
                    onclick="$('.tc input').prop('checked', false); $('#submit').prop('disabled', true);"
                  >
                    Uncheck All
                  </button>
                </div>
    ...
    <script>
      ...
      document.querySelector(".go-btn").addEventListener("click", function() {
        tables = [];
        $('.tc input').each(function(_, el) {
          if (el.checked) tables.push(+el.id.slice(1));
        });
      ...
      function setupRound() {
        ...
        questions = Object.keys(vals).reduce(
          (acc, item) =>
            acc.concat(
              { val: item, matchesTo: vals[item].join(" * ") },
              { val: vals[item].join(" * "), matchesTo: item }
            ),
          []
        );
      ...
      //   var link =
      //     "https://www.mathmammoth.com/practice/fractions-decimals#level=" +
      //     $("#gen-level").val() +
      //     "&tiles=" +
      //     $("#gen-tiles").val();
